...
const alreadyExists = async(req, res, next)=>{
    if(req.body.toUpdate) 
    {
        ...
        if(username!==req.username&&await User.findOne({username}).exec()) return res.status(403).json({'message' : 'username already in use'});  
        if(email!==req.email&&await User.findOne({email}).exec()) return res.status(403).json({'message' : 'email already in use'});  
        return next();
